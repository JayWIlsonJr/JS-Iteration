...
//Question Two
 var finalList = items.filter(function(item) {
    return item.price >= 14.0 && item.price <= 18.0;
 });
 console.log('Items that cost between $14.00 USD and $18.00 USD:' , finalList);



//Question Three
var object = items.filter(function(code) {
  return code.currency_code === "GBP";
});

var title = object.map(function(item) {
  return item.title;
});

var beerMugPrice = object.map(function(item) {
  return item.price;
});
...
//Question Four
var help = items.filter(function(item) {
  if (item.materials.indexOf("wood") >= 0) {
    return item;
    }
});

var finalOutput = help.forEach(function(item) {
...
//Question Five
var howMany = items.filter(function(item) {
  return item.materials.length > 8;
});

var helpMe = howMany.filter(function(item) {
  if (item.materials.length >= 8) {
    return item;
  }
});

var helpMeRightNow = helpMe.filter(function(item) {
...
//Question Six
var finalCount = items.filter(function(item) {
  return item.who_made == "i_did";
});

console.log(finalCount.length + ' were made by their sellers');
